		...
		local success, result = pcall(function()
				...
				----------------------------------------------------
				-- 🧹 CLEANUP PREVIOUS EXECUTION (Prevent Crash on Re-execute)
				----------------------------------------------------
				pcall(function()
					...
					print("[🧹] Previous zoom cleanup completed")
				end)
				...
				----------------------------------------------------
				-- ⚙️ SERVICES (Konsolidasi semua di sini)
				----------------------------------------------------
				...
				----------------------------------------------------
				-- 🔭 INFINITE ZOOM OUT (Always Active - Safe Re-execute)
				----------------------------------------------------
				do
					...
					print("[✅] Infinite Zoom Out enabled (Safe for re-execute)")
				end
				...
				----------------------------------------------------
				-- 🔄 AUTO RETURN ON RESET (Runs Automatically)
				----------------------------------------------------
				...
				player.CharacterAdded:Connect(function(character)
					...
					if savedCFrame then
						...
						print("[✓] Auto-teleported back to saved position!")
					else
				...
				----------------------------------------------------
				-- 🎣 MAIN TAB (Auto Legit + Instant v4 - Fluent Plus Safe)
				----------------------------------------------------
				local function LoadMainTab()
					...
					local CastRemote = NetPath:WaitForChild("RF/RequestFishingMinigameStarted")
					local EquipRemote = NetPath:WaitForChild("RE/EquipToolFromHotbar")

					-- State
					local autoLegitEnabled = false

					_G.ToggleLegit = MainSection_Legit:AddToggle("AutoLegitToggle", {
						Title = "Legit V2 (0 Click)",
						Description = "Auto perfect + Legit 0 Click 🎣",
						Default = false,
						Callback = function(Value)
							autoLegitEnabled = Value
							
							if Value then
								Fluent:Notify({Title = "Auto Fish", Content = "Started! Kamera ke air.", Duration = 3})
									...
									while autoLegitEnabled do
										...
										if activeMinigame then
											...
											if not FishingController:OnCooldown() then
												...
												local ChargeGui = Player.PlayerGui:FindFirstChild("Charge")
												...
												if not IsCharging then
													...
													if currentPower >= 0.91 then 
														local WaterY = 1.22232
														
														if WaterY then
															local ServerTime = Workspace:GetServerTimeNow()
															
															-- [PERBAIKAN UTAMA DI SINI]
															-- Kita tangkap 2 Return Value: (Success, Data)
															game:GetService("Players").LocalPlayer.PlayerGui.Charge.Enabled = false
															local Success, MinigameData = CastRemote:InvokeServer(WaterY, 1.0, ServerTime)
															
																print("✅ Server Accepted & Data Received!")
																
																-- Masukkan Data (Table) ke Controller, BUKAN Success (Boolean)
																...
																if AnimController then
																	AnimController:StopAnimation("StartRodCharge")
																end
															task.wait(0.3)
														else
								...
								Fluent:Notify({Title = "Auto Fish", Content = "Stopped.", Duration = 3})
								FishingController:RequestClientStopFishing(true)
							end
						end
					})
					...
					----------------------------------------------------
					-- ⚡ INSTANT FISHING v4.1 OPTIMIZED (No Animation + GUI Toggle)
					-- Semua delay tetap sama, anim balik normal saat toggle off
					----------------------------------------------------
					
					print("=== ⚡ Instant Fishing v4.1 (Optimized No Animation) Loaded ===")
					...
					if not net then
						warn("❌ Tidak menemukan net package (sleitnick_net@0.2.0)")
						return
					end
					...
					if not ReplionPackage then 
						return warn("❌ Folder Replion tidak ditemukan!") 
					end
					...
					if getgenv().UIKillerConnection then
						...
						print("🔄 UI Killer lama dimatikan, memulai ulang...")
					end
					...
					local function startOldInstantFishing()
						ThreadInstantV4 = task.spawn(function()
							...
							enableAnimations()
							print("🛑 Instant Fishing v4 stopped & animation restored.")
						end)
					end
					...
					----------------------------------------------------
					-- 🧩 GUI Integration (Fluent)
					----------------------------------------------------
					_G.ToggleInstant = MainSection_Instant:AddToggle("InstantFishing", {
						Title = "Instant Fishing Mode",
						Description = "Super fast mode — skips normal fishing steps and instantly catches fish. (High speed, use carefully!)",
						Default = false,
						Callback = function(Value)
							_G.InstantV4 = Value
							if Value then
								...
								if getgenv().UIKillerConnection then
									...
									print("🛑 UI Killer BERHASIL Dimatikan!")
								else
									warn("⚠️ Tidak ada UI Killer yang sedang berjalan.")
								end
							end
						end
					})
					...
					print("✅ Instant Fishing v4.1 (Optimized + Toggle GUI) Loaded.")
					...
					----------------------------------------------------
					-- 💀 BLATANT MODE (Soul Scythe Exclusive) v1.0
					...
					if REReplicateTextEffect then
						...
						print("✅ Exclaim auto-trigger connected")
					else
						warn("⚠️ ReplicateTextEffect remote not found!")
					end
					...
					local function stopFishing()
						...
						print("🛑 Fishing stopped & cancelled 3x")
					end
					
					----------------------------------------------------
					-- 🧩 GUI Integration (Fluent) - Tab Main
					----------------------------------------------------
					...
					print("✅ Blatant Mode (Soul Scythe) v1.0 Loaded - Ready to dominate! 💀")
					...
					_G.ToggleBlantantV2 = MainSection_BlatantV2:AddToggle("BlatantV2Toggle", {
						...
						Callback = function(Value)
							...
							if Value then
								game:GetService("StarterGui"):SetCore("SendNotification", {
									Title = "Blatant Fishing", Text = "Status: ON ✅", Duration = 3
								})
								...
								game:GetService("StarterGui"):SetCore("SendNotification", {
									Title = "Blatant Fishing", Text = "Status: OFF 🛑", Duration = 3
								})
							end
						end
					})
					...
					local function AutoEnchantRod()
						while _G.AutoEnchant do
							...
							if stoneUUID then
								...
								print("⏳ Menunggu hasil Roll (Arg 2)...")
								...
								print("❌ Batu enchant tidak ditemukan di inventory.")
				...
				local function LoadShopTab()
					...
					----------------------------------------------------
					-- ⭐ AUTO FAVORITE (FINAL FIXED LOGIC)
                    ...
                    -- [TAMBAHAN WAJIB] Module untuk Real Tier
                    local TierUtility = require(ReplicatedStorage.Shared.TierUtility) 

                    ----------------------------------------------------
                    -- 🐟 FISH CACHE & REAL TIER CALCULATION
                    ----------------------------------------------------
                    local AvailableFishNames = {}
                    local ItemCache = {}     -- Menyimpan Static Data
                    local VariantCache = {}  -- Menyimpan Nama Variant
                    local RealTierCache = {} -- [BARU] Menyimpan Tier Asli (1-7) berdasarkan ID Ikan

                    local function InitializeSystem()
                        local ItemsFolder = ReplicatedStorage:WaitForChild("Items")
                        local scannedNames = {}
                        
                        -- 1. Scan Variant (Mutation) Names
                        ...
                        -- 2. Scan Items & Calculate Real Tier
                        for _, itemModule in ipairs(ItemsFolder:GetChildren()) do
                            if itemModule:IsA("ModuleScript") then
                                local success, itemData = pcall(function() return require(itemModule) end)
                                ...
                                if success and itemData and itemData.Data then
                                    ...
                                    -- Hanya proses Ikan
                                    if d.Type == "Fish" and d.Name and d.Id then
                                        
                                        -- A. Simpan ke Cache Item
                                        ItemCache[d.Id] = itemData
                                        
                                        -- B. Simpan Nama untuk UI
                                        if not scannedNames[d.Name] then
                                            table.insert(AvailableFishNames, d.Name)
                                            scannedNames[d.Name] = true
                                        end
                                        
                                        -- C. [LOGIKA REAL TIER]
                                        local finalTier = d.Tier or 1 -- Default ke Tier bawaan
                                        
                                        -- Jika ada data Probability, kita hitung ulang Tier-nya
                                        ...
                                        -- Simpan Tier Asli ke Cache khusus
                                        RealTierCache[d.Id] = finalTier
                                    end
                                end
                            end
                        end
                        ...
                        print(string.format("[AutoFav] System Initialized. Cached %d Fish Tiers.", #AvailableFishNames))
                    end
                    ...
                    local function isTierSelected(realTier)
                        -- Kita cek Tier Angka (1-7) dengan list yang dipilih user
                        for _, id in pairs(selectedRarities) do
                            if id == realTier then return true end
                        end
                        return false
                    end
                    ...
                    local function isFishNameSelected(fishId)
                        ...
                        local itemData = ItemCache[fishId]
                        if not itemData or not itemData.Data.Name then return false end
                        
                        local fishName = itemData.Data.Name
                        for _, selectedName in pairs(selectedFishNames) do
                            if fishName == selectedName then return true end
                        end
                        return false
                    end

                    ----------------------------------------------------
                    -- 🧠 3. SCANNING LOGIC (UPDATED WITH REAL TIER)
                    ----------------------------------------------------
                    ...
                    local function ScanInventoryForFavorites()
                        ...
                        for _, invItem in pairs(inventory.Items) do
                            ...
                            if not invItem.Favorited then
                                -- Cek apakah ID item ini ada di cache RealTier kita (berarti dia Ikan)
                                local realTier = RealTierCache[invItem.Id]
                                
                                if realTier then -- Jika ada nilainya, berarti ini Ikan Valid
                                    ...
                                    -- [LOGIC 2] Cek Real Tier (UPDATED)
                                    -- Sekarang kita pakai 'realTier' dari cache perhitungan, bukan static data
                                    if not shouldFavorite and isTierSelected(realTier) then
                                        shouldFavorite = true
                                    end
                                    ...
                                    if not shouldFavorite and invItem.Metadata then
                                        ...
                                        if invItem.Metadata.VariantId then
                                            ...
                                            if cacheName and isMutationSelected(cacheName) then mutationFound = true end
                                        end
                                        
                                        -- Fallbacks
                                        if not mutationFound and isMutationSelected(invItem.Metadata.VariantId) then mutationFound = true end
                                        if not mutationFound and invItem.Metadata.Mutation and isMutationSelected(invItem.Metadata.Mutation) then mutationFound = true end

                                        if mutationFound then shouldFavorite = true end
                                    end
                                    
                                    -- Jika lolos seleksi, masukkan antrian favorite
                            ...
                            while autoFavEnabled do
                                ...
                                if #itemsToFav > 0 then
                                    for _, uuid in pairs(itemsToFav) do
                                        ...
                                        favHandler(uuid)
                                        task.wait(0.2) -- Delay aman agar tidak spam remote parah
                                    end
                                else
                                    task.wait(1)
                                end
                    ...
                    Auto:AddToggle("AutoFavoriteToggle", {
                        ...
                        Description = "Uses Real Tier Calculation 📊",
                        Default = false,
                        Callback = function(Value)
                    ...
                    Auto:AddDropdown("FavFishName", {
                        ...
                        Description = "Choose specific fish (Highest Priority)",
                        ...
                        Callback = function(Value)
                            ...
                            for fishName, isSelected in pairs(Value) do
                                if isSelected then table.insert(selectedFishNames, fishName) end
                            end
                        end
                    })

                    Auto:AddDropdown("FavRarity", {
                        Title = "Fish Rarities (Real Tier)",
                        Description = "Select rarities (Calculated from Probability)",
                    ...
                    Auto:AddDropdown("FavMutation", {
                        ...
                        Callback = function(Value)
                            ...
                            for mutation, isSelected in pairs(Value) do
                                if isSelected then table.insert(selectedMutations, mutation) end
                            end
                        end
                    })
							...
							AutoFavSection:AddButton({
								...
								Description = "Reset Favorites (Hati-hati!)",
								Callback = function()
									-- Pastikan variabel 'Window' sesuai dengan nama variabel window Fluent kamu
									Window:Dialog({
										Title = "Konfirmasi Hapus",
										Content = "Yakin ingin UNFAVORITE semua ikan? Proses ini butuh waktu.",
										Buttons = {
											{
												Title = "Ya, Lakukan",
												Callback = function()
													print("[AutoFav] Memulai proses Unfavorite...")
													
													task.spawn(function()
														-- Ambil Data Terbaru
														local playerData = Replion.Client:WaitReplion("Data", 5)
														if not playerData then 
															Fluent:Notify({Title="Error", Content="Gagal mengambil data inventory", Duration=3})
															return 
														end
														...
														for _, invItem in pairs(inventory.Items) do
															-- Cek apakah item ini di-Favorite
															if invItem.Favorited then
																
																-- [FIX ERROR DISINI]
																-- Ganti getItemStaticData(invItem.Id) menjadi ItemCache[invItem.Id]
																-- Pastikan ItemCache sudah didefinisikan di script utama kamu sebelumnya
																local staticData = ItemCache[invItem.Id]
																
																-- Jika ItemCache nil, kita coba require manual (Safety fallback)
																if not staticData then
																	pcall(function()
																		local itemMod = ReplicatedStorage.Items:FindFirstChild(tostring(invItem.Id))
																		if itemMod then staticData = require(itemMod) end
																	end)
																end
																
																if staticData and staticData.Data and staticData.Data.Type == "Fish" then
																	-- Eksekusi Unfavorite
																	favHandler(invItem.UUID)
																	count = count + 1
																	
																	-- Delay agar tidak disconnect/remote drop
																	task.wait(0.15) 
																end
															end
														end
														
														print("[AutoFav] Selesai! Total Unfavorited: ", count)
														Fluent:Notify({Title="Selesai", Content="Berhasil unfavorite "..count.." ikan.", Duration=5})
													end)
												end
											},
											{
					...
					-- 	if NoclipLoop then
					-- 		NoclipLoop:Disconnect() -- ⚠️ MEMUTUS LOOP DARI GAME ⚠️
					...
					local function GetNotificationRemote()
						...
						if success and result then
							...
							warn("❌ Remote TextNotification tidak ditemukan! Cek path folder.")
							return nil
						end
					end
					local onTrade = false
					...
					print("[✓] Auto Trade System loaded!")
					...
					if rootPart then
						...
						print("📌 Posisi awal disimpan. Script akan kembali ke sini nanti.")
					else
						warn("❌ Karakter tidak ditemukan! Script dibatalkan.")
						return
					end
							...
							if treasureObject then
								...
								if targetCFrame then
									...
									if prompt then
										...
										-- warn("⚠️ Tombol tidak ada di Treasure #" .. foundCount)
									end
									
									-- Jeda sedikit
									task.wait(0.3)
								else
									warn("⚠️ Gagal mendapatkan posisi Treasure #" .. foundCount)
								end
							end
						end
					end
    ...
    local function collectCrystals()
		...
		local oldInstant = _G.ToggleInstant.Value
		local oldBlantant = _G.ToggleBlantant.Value
        ...
        getgenv().CrystalJobConnection = NotificationRemote.OnClientEvent:Connect(function(data)
            if type(data) == "table" and data.Text then
                if string.find(string.lower(data.Text), "crystals are glowing") then
					...
					LimitedEvent:AddToggle("AutoCrystal", {
						...
						Callback = function(Value)
							if Value then
								...
								Fluent:Notify({
									Title = "Auto Crystal",
									Content = "Auto Mining Crystal diaktifkan.",
									Duration = 3
								})
							else
								if getgenv().CrystalJobConnection then
								...
								Fluent:Notify({
									Title = "Auto Crystal",
									Content = "Auto Mining Crystal dimatikan.",
									Duration = 3
								})
							end
						end
					...
					local function startAncient()
						...
						AncientThread = task.spawn(function()
							...
							local teleSpot = true
							print("Teleporting to Ancient Ruin Event...")
							while true do
								local text = workspace["!!! DEPENDENCIES"]["Event Tracker"].Main.Gui.Content.Items.Stats.Timer.Label.Text
									...
									while true do
										if selectedPlayerName ~= "" and selectedPlayerName ~= "No other players" then
											...
											if targetPlayer and targetPlayer.Character then
												local targetCFrame = GetTargetCFrame(selectedPlayerName)
												local myChar = player.Character
												local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")

												if targetCFrame and myHrp then
				...
				local function LoadVulnTab()
					...
					local function openGift()
						...
						if success then
							print("✅ Target set! Purchase prompt should open...")
						else
				...
				----------------------------------------------------
				-- ⚙️ PERFORMANCE TAB - EXTREME LOW GRAPHICS V2
				----------------------------------------------------
				local function LoadPerformanceTab()
					...
					PerformanceSection:AddToggle("TextureNukeSaver", {
						...
						Callback = function(Value)
							...
							if Value then
								...
								print("⚠️ Harap Rejoin server untuk mengembalikan Texture/Model.")
							end
						end
					})
					...
					local boosterEnabled = false
					local connections = {} -- Menyimpan event listener

					local Players = game:GetService("Players")
					local RunService = game:GetService("RunService")

					-- Simpan koneksi agar bisa dimatikan nanti
					local AnimConnections = {} 

					-- 🛠️ FUNGSI MATIKAN ANIMASI KARAKTER
					local function FreezeCharacter(character)
						if not character then return end
						
						-- Tunggu Humanoid sebentar
						local hum = character:WaitForChild("Humanoid", 2)
						if not hum then return end

						-- 1. Matikan Script "Animate" bawaan Roblox (biar ga error spam console)
						...
						-- 2. Hancurkan "Animator" (Otak penggerak tulang)
						local animator = hum:FindFirstChildOfClass("Animator")
						if animator then
							-- Stop dulu semua track yang sedang jalan biar ga nyangkut aneh
							pcall(function()
								for _, track in pairs(animator:GetPlayingAnimationTracks()) do
									track:Stop()
								end
							end)
							animator:Destroy() -- HAPUS TOTAL
						end
					end

					-- 🛠️ FUNGSI UTAMA (PANGGIL INI DI DALAM applyBooster)
					local function RemoveAllAnimations()
						-- 1. Matikan animasi Player Sendiri
						if Players.LocalPlayer.Character then
							FreezeCharacter(Players.LocalPlayer.Character)
						end
						-- Kalau kita mati/respawn, matikan lagi
						local myConn = Players.LocalPlayer.CharacterAdded:Connect(FreezeCharacter)
						table.insert(AnimConnections, myConn)

						-- 2. Matikan animasi Player Lain yang SUDAH ADA di server
						for _, plr in pairs(Players:GetPlayers()) do
							if plr ~= Players.LocalPlayer and plr.Character then
								FreezeCharacter(plr.Character)
							end
							
							-- Kalau mereka respawn, matikan lagi
							local conn = plr.CharacterAdded:Connect(FreezeCharacter)
							table.insert(AnimConnections, conn)
						end

						-- 3. Matikan animasi Player BARU yang baru join
						local addedConn = Players.PlayerAdded:Connect(function(plr)
							local conn = plr.CharacterAdded:Connect(FreezeCharacter)
							table.insert(AnimConnections, conn)
						end)
						table.insert(AnimConnections, addedConn)
					end

					-- 🛠️ FUNGSI RESTORE (Opsional, tapi animasi biasanya butuh Rejoin/Respawn buat bener lagi)
					local function RestoreAnimations()
						for _, conn in pairs(AnimConnections) do
							conn:Disconnect()
						end
						AnimConnections = {}
						-- Catatan: Animator yang sudah di-Destroy gabisa balik kecuali respawn.
					end

					-- 🛠️ FUNGSI BRUTAL: PEMBERSIH OBJEK
					local function brutalClean(obj)
						if not boosterEnabled then return end

						pcall(function()
							-- 1. Hapus Texture & Decal (Gambar Tempelan)
							if obj:IsA("Texture") or obj:IsA("Decal") or obj:IsA("ShirtGraphic") then
								obj:Destroy()
							
							-- 2. Hapus Baju & Celana (Karakter jadi polos warna)
							elseif obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("Clothing") then
								obj:Destroy()

							-- 3. Hapus Efek Partikel (Api, Asap, Sparkles)
							elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Smoke") 
								or obj:IsA("Fire") or obj:IsA("Sparkles") or obj:IsA("Beam") or obj:IsA("Explosion") then
								obj.Enabled = false
								obj:Destroy()
							
							-- 4. Hapus Efek Post-Processing (Blur, SunRays, Bloom)
							elseif obj:IsA("PostEffect") or obj:IsA("Atmosphere") or obj:IsA("Sky") or obj:IsA("SunRaysEffect") or obj:IsA("DepthOfFieldEffect") then
								obj:Destroy()

							-- 5. Optimasi BasePart (Balok/Lantai/Dinding)
							elseif obj:IsA("BasePart") then
								...
								-- Ubah warna jadi abu-abu agar ringan dirender GPU
								if obj.Transparency == 0 then
									obj.Color = Color3.fromRGB(100, 100, 100) 
								end

							-- 6. Optimasi MeshPart (Benda bentukan 3D)
							elseif obj:IsA("MeshPart") then
								obj.TextureID = "" -- Hapus texture mesh
								obj.Material = Enum.Material.SmoothPlastic
								obj.Reflectance = 0
								obj.CastShadow = false
							
							-- 7. Optimasi SpecialMesh
					...
					local function applyBooster()
						...
						-- 1️⃣ SETTING LIGHTING MATI TOTAL (Fullbright Flat)
						pcall(function()
							Lighting.GlobalShadows = false
							Lighting.FogStart = 9e9 -- Hapus kabut
							Lighting.FogEnd = 9e9
							Lighting.Brightness = 2
							Lighting.ClockTime = 14 -- Siang hari mati (Matahari diam)
							Lighting.OutdoorAmbient = Color3.fromRGB(128, 128, 128)
							Lighting.Ambient = Color3.fromRGB(128, 128, 128)
							Lighting.EnvironmentDiffuseScale = 0
							Lighting.EnvironmentSpecularScale = 0
							Lighting.Technology = Enum.Technology.Compatibility -- Teknologi render paling purba (ringan)
							
							-- Hapus semua anak Lighting (Skybox, Atmosphere, dll)
							for _, v in pairs(Lighting:GetChildren()) do
								v:Destroy()
							end
						end)

						-- 2️⃣ TERRAIN JADI DATAR (Air jadi blok padat)
						pcall(function()
							if Terrain then
								Terrain.WaterWaveSize = 0
								Terrain.WaterWaveSpeed = 0
								Terrain.WaterReflectance = 0
								Terrain.WaterTransparency = 0 -- Air jadi solid (tidak transparan)
								Terrain.Decoration = false
								Terrain.MaterialColors = {
									Grass = Color3.fromRGB(100, 100, 100),
									Sand = Color3.fromRGB(100, 100, 100),
									Water = Color3.fromRGB(100, 100, 100),
								}
							end
						end)

						task.spawn(RemoveAllAnimations)

						-- 3️⃣ LOOPING SEMUA BENDA DI GAME (BRUTAL SCAN)
						-- Menggunakan task.spawn agar game tidak freeze saat proses
						task.spawn(function()
							for _, obj in pairs(workspace:GetDescendants()) do
								brutalClean(obj)
								if obj:IsA("BasePart") then removedCount = removedCount + 1 end
								if removedCount % 1000 == 0 then task.wait() end -- Jeda setiap 1000 objek biar ga crash
							end
							
							-- Notifikasi Selesai (Sesuaikan dengan library UI kamu, WindUI/Fluent)
							-- Contoh WindUI:
							-- Window:Notify({Title="Brutal Booster", Content="Optimized "..removedCount.." objects!", Duration=3})
						end)

						-- 4️⃣ MENCEGAH TEXTURE BARU MUNCUL (Auto-Delete New Parts)
						local conn = workspace.DescendantAdded:Connect(function(obj)
							if boosterEnabled then
								task.wait() -- Tunggu sebentar agar property ter-load
								brutalClean(obj)
							end
						end)
						table.insert(connections, conn)
						
						-- 5️⃣ SETTING RENDERING BAWAAN ROBLOX
						pcall(function()
					...
					local function restoreBooster()
						...
						-- Kembalikan Setting Lighting (Standar)
						pcall(function()
							Lighting.GlobalShadows = true
							Lighting.Brightness = 2
							Lighting.ClockTime = 14
							Lighting.Technology = Enum.Technology.Future -- Atau ShadowMap
							Lighting.EnvironmentDiffuseScale = 1
							Lighting.EnvironmentSpecularScale = 1
						end)

						-- Kembalikan Air (Standar)
						pcall(function()
							if Terrain then
						...
						-- ⚠️ CATATAN: Texture yang sudah di-Destroy TIDAK BISA kembali.
						-- User harus rejoin untuk melihat texture lagi.
					end

					----------------------------------------------------
					-- MASUKKAN INI KE TOGGLE UI KAMU
					----------------------------------------------------
					-- Contoh implementasi pada Toggle:

					PerformanceSection:AddToggle("ExtremeBooster", {
						Title = "FPS BOOSTER",
						Description = "Hapus SEMUA Texture, Efek & Lighting. (Wajib Rejoin untuk normal kembali)",
						Default = false,
						Callback = function(Value)
							if Value then
								applyBooster()
					...
					----------------------------------------------------
					-- 🎣 TOGGLE: Disable / Enable Rod Effects (VFX)
					----------------------------------------------------
					...
					PerformanceSection:AddToggle("DisableRodVFX", {
						...
						Callback = function(Value)
							...
							if not VFXUtility then
								warn("⚠️ Module VFXUtility tidak ditemukan.")
								return
							end
					...
					----------------------------------------------------
					-- 🔕 TOGGLE: Disable Notifications (Small Notification)
					----------------------------------------------------
					...
					PerformanceSection:AddToggle("DisableNotifications", {
						...
						Callback = function(Value)
							...
							if Value then
								-- ✅ Disable notification GUI & block future ones
								task.spawn(function()
					...
					----------------------------------------------------
					-- 🎬 TOGGLE: Disable Cutscenes (Skip Cinematics)
					----------------------------------------------------
					...
					----------------------------------------------------
					-- 🎣 TOGGLE: Disable Fishing Animation (Use Instant v4 Function)
					----------------------------------------------------
						...
						Callback = function(Value)
							if Value then
								if disableAnimations then
									...
									warn("⚠️ disableAnimations() not found. Make sure Instant Fishing v4 is loaded.")
								end
							else
								if enableAnimations then
									enableAnimations()
									Fluent:Notify({
										Title = "🎞️ Animations Restored",
										Content = "Fishing animations restored via Instant Fishing v4.",
										Duration = 4
									})
								else
									warn("⚠️ enableAnimations() not found.")
								end
							end
						end
					})
				...
				local function LoadQuestTab()
					...
					-- local LeviathanSection = QuestTab:AddSection("Leviathan Quest")

					-- local LeviathanThread = nil
					-- local LeviathanProgress = {
					-- 	ForgottenActive = false,
					-- 	ForgottenScale = false,
					-- 	LeviathanScale = false,
					-- 	LeviathanHunt = false,
					-- 	RelicsAvailable = false,
					...
					-- local PlateNumbers = {"1", "2", "3"}

					-- ------------------------------------------------------
					-- -- 🛡️ FUNGSI SAFE TELEPORT (ANTI CRASH)
					-- ------------------------------------------------------
					...
					-- 		-- 1. Bekukan Fisika (Mencegah Crash Tabrakan)
					-- 		Root.AssemblyLinearVelocity = Vector3.zero
					-- 		Root.AssemblyAngularVelocity = Vector3.zero
					-- 		Root.Anchored = true
							
					-- 		-- 2. Load Map Dulu (Mencegah Crash Void)
					-- 		pcall(function()
					-- 			LocalPlayer:RequestStreamAroundAsync(targetPos)
					-- 		end)
							
					-- 		-- 3. Teleport dengan Offset (Di atas objek)
					-- 		Root.CFrame = CFrame.new(targetPos + Vector3.new(0, 4, 0))
							
					-- 		-- 4. Wajib Tunggu (Sinkronisasi Server)
					-- 		task.wait(0.5)
					...
					-- 	print("--- 🟢 MEMULAI PENGAMBILAN RELIC ---")
					-- 	for _, name in ipairs(RelicNames) do
					-- 		pcall(function()
					-- 			local RelicObject = Workspace.Camera:FindFirstChild(name)
					-- 			if RelicObject then
					-- 				local Attachment = RelicObject:FindFirstChild("Attachment")
					-- 				if Attachment then
					-- 					local FixedPos = Attachment.WorldPosition
										
					-- 					-- Cek Validitas Koordinat
					-- 					if FixedPos.X == FixedPos.X then
					-- 						print("🚀 OTW Relic: " .. name)
					-- 						SafeTeleport(FixedPos)
					...
					-- 				end
					-- 			else
					-- 				print("⚠️ Skip Relic (Tidak ketemu): " .. name)
					-- 			end
					-- 		end)
					-- 		task.wait(1) -- Jeda aman
					-- 	end
					-- 	print("--- 🏁 RELIC SELESAI ---")
					-- end

					-- ------------------------------------------------------
					-- -- ⚡ BAGIAN 2: PRESSURE PLATES
					-- ------------------------------------------------------
					-- local function ActivatePlates()
					-- 	print("--- 🟢 MEMULAI PRESSURE PLATES ---")
						
					-- 	local LeviathanEvent = Workspace:FindFirstChild("LeviathanEvent")
					-- 	if not LeviathanEvent then
					-- 		warn("❌ LeviathanEvent Folder tidak ditemukan!")
					-- 		return
					-- 	end

					-- 	local PlatesFolder = LeviathanEvent:FindFirstChild("PressurePlates")
					-- 	if not PlatesFolder then
					-- 		warn("❌ Folder PressurePlates tidak ditemukan!")
					-- 		return
					-- 	end

					-- 	for _, num in ipairs(PlateNumbers) do
					-- 		pcall(function()
					-- 			-- Cari Plate ("1", "2", "3")
					-- 			local PlateModel = PlatesFolder:FindFirstChild(num)
								
					-- 			if PlateModel then
					-- 				-- Cari Part di dalamnya
					-- 				local TargetPart = PlateModel:FindFirstChild("Part")
									
					-- 				if TargetPart then
					-- 					print("🚀 OTW Plate Nomor: " .. num)
										
					-- 					-- Ambil posisi Part
					-- 					local PlatePos = TargetPart.Position
										
					-- 					-- Teleport Aman
					-- 					SafeTeleport(PlatePos)
										
					-- 					-- Cari Prompt di dalam Part
					-- 					local Prompt = TargetPart:FindFirstChild("ProximityPrompt")
					-- 					if Prompt then
					-- 						-- Kadang perlu delay dikit biar karakter napak
					-- 						task.wait(0.2)
					-- 						fireproximityprompt(Prompt)
					-- 						print("✅ AKTIFKAN Plate: " .. num)
					-- 					else
					-- 						warn("⚠️ Tidak ada Prompt di Plate " .. num)
					-- 					end
					-- 				else
					-- 					warn("❌ Plate " .. num .. " tidak memiliki 'Part'")
					-- 				end
					-- 			else
					-- 				warn("❌ Plate " .. num .. " tidak ditemukan.")
					-- 			end
					-- 		end)
							
					-- 		-- Jeda antar plate (Penting biar gak ke-detect spam)
					-- 		task.wait(1.5)
					-- 	end
					...
					-- 	-- Gunakan :FindFirstChild agar tidak error kalau barangnya hilang
					-- 	if Cam:FindFirstChild("Sunken Eye Relic") or Cam:FindFirstChild("Burntflame Relic") or Cam:FindFirstChild("Blacktide Relic") then
					-- 		LeviathanProgress.RelicsAvailable = true
					-- 	else
					-- 		LeviathanProgress.RelicsAvailable = false
					-- 	end

					-- 	if game:GetService("Players").LocalPlayer.PlayerGui.Events.Frame.Events["Leviathan Hunt"].Visible then
					-- 		LeviathanProgress.LeviathanHunt = true
					-- 	else
					-- 		LeviathanProgress.LeviathanHunt = false
					-- 	end
					...
					-- 	for _, invItem in pairs(inventory.Items) do
					-- 			if invItem.Id == 576 then
					-- 				LeviathanProgress.LeviathanScale = true
					-- 				LeviathanProgress.ForgottenScale = true
					-- 				break
					-- 			end
					-- 	end
					...
					-- 	local targetQuestText = "Forgotten Scale"
					...
					-- 			LeviathanProgress.ForgottenActive = true
					-- 			break
					-- 		end
					-- 	end
					-- end
					...
					-- 	checkProgress()
					-- 	if LeviathanProgress.DoorOpened then 
					-- 	elseif LeviathanProgress.LeviathanHunt then
					-- 		if LeviathanProgress.RelicPlaced then
					-- 		elseif LeviathanProgress.RelicCollected then
					-- 			ActivatePlates()
					-- 		elseif LeviathanProgress.RelicsAvailable then
					-- 			CollectRelics()
					-- 		end

					-- 	elseif LeviathanProgress.LeviathanScale then
					-- 		print("skipped")
					-- 		local args = {
					-- 			"e14efb98-571b-49ee-acb4-65a899555835"
					-- 		}
					-- 		game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"):WaitForChild("RF/ConsumeItem"):InvokeServer(unpack(args))

					-- 	elseif not LeviathanProgress.ForgottenActive then
					-- 		print("Memulai Quest Forgotten Scale...")
					-- 		local args = {
					-- 			"Archaeologist",
					-- 			1,
					-- 			3
					...
					-- LeviathanSection:AddToggle("LeviathanDen", {
					-- 	Title = "Auto Leviathan Hunt",
					-- 	Description = "Auto quest leviathan hunt.",
					-- 	Default = false,
					-- 	Callback = function(Value)
					-- 		if Value then
					...
					-- 				Fluent:Notify({
					-- 					Title = "Leviathan Quest Enabled",
					-- 					Content = "Auto Leviathan Hunt started.",
					-- 					Duration = 4
					-- 				})
					-- 		else
					-- 			task.cancel(LeviathanThread)
					-- 				Fluent:Notify({
					-- 					Title = "Leviathan Quest Disabled",
					-- 					Content = "Auto Leviathan Hunt stopped.",
					-- 					Duration = 4
					-- 				})
					-- 		end
					-- 	end
					...
					local function SearchByPosition()
						local count = 0
						validArrow, validHourglass, validCrescent, validDiamond = nil, nil, nil, nil
						...
						for _, model in pairs(Folder:GetChildren()) do
							...
							if model:IsA("Model") then
								...
								if root and prompt then
									...
									if (pos - FixedPositions.Arrow).Magnitude < 10 then
										validArrow = prompt
										
									elseif (pos - FixedPositions.Hourglass).Magnitude < 10 then
										validHourglass = prompt
										
									elseif (pos - FixedPositions.Crescent).Magnitude < 10 then
										validCrescent = prompt
										
									elseif (pos - FixedPositions.Diamond).Magnitude < 10 then
										validDiamond = prompt
									end
								end
							end
					...
					local function UpdateLeverUI()
						...
						if not validArrow then
							LeverUi["Arrow"]:SetTitle("✅ Arrow Lever Activated")
						else
							LeverUi["Arrow"]:SetTitle("⬜ Arrow Lever Inactive")
						end

						if not validHourglass then
							LeverUi["Hourglass Diamond"]:SetTitle("✅ Hourglass/Diamond Lever Activated")
						else
							LeverUi["Hourglass Diamond"]:SetTitle("⬜ Hourglass/Diamond Lever Inactive")
						end

						if not validCrescent then
							LeverUi["Crescent"]:SetTitle("✅ Crescent Lever Activated")
						else
							LeverUi["Crescent"]:SetTitle("⬜ Crescent Lever Inactive")
						end

						if not validDiamond then
							LeverUi["Diamond"]:SetTitle("✅ Diamond Lever Activated")
						else
							LeverUi["Diamond"]:SetTitle("⬜ Diamond Lever Inactive")
						end
					end
					...
					local function placeArtifactLevers(val)
						...
						if success then
							print(val)
							print("✅ Berhasil menaruh Artifact!")
						else
					...
					local function UpdateDiamondQuestUI()
						...
						if not IsDiamondQuestActive() then
							DiamondQuestUI[1]:SetTitle("⚠️ Quest Not Active")
						...
						if not Replion or not QuestDefModule then
							-- Jika gagal load, beri info di UI
							DiamondQuestUI[1]:SetTitle("❌ Error")
					...
					local function AutoDiamondQuest()
						...
						while true do
							...
							if not IsDiamondQuestActive() then
								print("❌ Diamond Researcher quest tidak aktif! Mencoba mengambil quest...")
					...
					function KaitunUI.Update()
						...
						local function SetCheck(label, name, hasItem)
							if label then
								label.Text = name .. ": " .. (hasItem and "✅" or "❌")
									...
									if not KaitunData.ElementRod then
										if not KaitunData.GhostfinnRod then
											if not KaitunData.Midnight and KaitunData.Coins <= 1000000 then
												...
												if KaitunData.Coins >= 350 then
													KaitunData.Status = "Upgrading to Luck Rod..."
													local RFPurchaseFishingRod = ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RF/PurchaseFishingRod"]
				...
				----------------------------------------------------
					-- 🏷️ TAB WEBHOOK (Real Tier & Image by Name Fixed)
					----------------------------------------------------
				...
				local TierUtility = require(RS.Shared.TierUtility) -- Module Wajib untuk Real Tier

				local TierMap = {[1]="Common",[2]="Uncommon",[3]="Rare",[4]="Epic",[5]="Legendary",[6]="Mythic",[7]="SECRET"}
				local FishData = {}
				local FishImages = {}
				local FishNameMap = {} -- Mapping Nama -> ID
				local RecentLogs = {}  -- Cache untuk Anti-Duplicate (Opsional tapi bagus)
				local SortedFishCache = {} -- [BARU] Untuk menyimpan list ikan yang sudah diurutkan panjangnya
				local WebhookState = {
					enabled=false, 
					url="", 
					rarities={}, 
					connections={}, -- Array untuk menyimpan koneksi
				...
				-- Pastikan JSON di Github memiliki format: [{"name": "Fish Name", "imageUrl": "link..."}, ...]
						...
						if ok and response and response.Body then
							...
							if success and data then
								for _, fish in pairs(data) do
									if fish.name and fish.imageUrl then
										FishImages[fish.name] = fish.imageUrl
									end
								end
								print("✅ [Webhook] Fish Images Loaded via Name")
							end
						end
					end)
				end

				-- [2] Scan Fish with Real Tier Calculation
				local function ScanFish()
					...
					FishData = {}
					FishNameMap = {}
					SortedFishCache = {} -- Reset list urutan
						...
						if m:IsA("ModuleScript") then
							...
							if ok and data and data.Data then
								...
								if d.Id and d.Name then
									...
									local realTier = d.Tier or 1
									if data.Probability and data.Probability.Chance and TierUtility then
										local tierInfo = TierUtility:GetTierFromRarity(data.Probability.Chance)
										if tierInfo and tierInfo.Tier then
											realTier = tierInfo.Tier
										end
									end
									...
									FishNameMap[tostring(d.Name)] = d.Id
									
									-- [BARU] Masukkan ke list sementara untuk sorting
									table.insert(SortedFishCache, {id = d.Id, name = tostring(d.Name)})
								end
							end
						end
					end
					
					-- [BARU] SORTING LOGIC: Urutkan dari nama TERPANJANG ke TERPENDEK
					-- Ini kunci agar "Ancient Squid" terdeteksi sebelum "Squid"
					table.sort(SortedFishCache, function(a, b)
						return #a.name > #b.name
					end)
					
					print("✅ [Webhook] Fish Data Scanned & Sorted by Length")
				end

				-- [3] Helper: Kirim Webhook ke Discord
				local function SendWebhook(fishId, weight, mutation, targetPlayer)
					...
					elseif type(weight) == "string" and weight ~= "" then
						weightStr = weight
					end
					...
					local embed = {
						...
						footer = {text="iSylHub Premium • Mode: "..WebhookState.mode, icon_url=LOGO_URL},
				...
				-- [4] Helper: Parse System Message (Revised for Weight & Full Name)
				local function ParseSystemMessage(msg)
					if not string.find(msg, "obtained a") then return nil end
					
					-- 1. Ambil Nama Player
					local playerName = msg:match("</b>%s*(.-)%s*obtained a")
					if not playerName then return nil end
					
					-- 2. Ambil Bagian Ikan Lengkap (Nama + Berat)
					local fishPart = msg:match("obtained a.-color=\".-\">(.-)</font>")
					if not fishPart then return nil end
					
					print("Debug Fish Part: "..fishPart)
					-- fishPart contoh: "Galaxy Big Ancient Squid (8.8K kg)"
					
					-- 3. [FIX] Pisahkan Nama dan Berat menggunakan Pattern Anchoring
					-- Pattern: "^(.-)" = Ambil semua text dari depan (Nama)
					--          "%s*%((.-)%)$" = Sampai ketemu spasi dan kurung di PALING BELAKANG (Berat)
					local fullFishName, weight = fishPart:match("^(.-)%s*%((.-)%)$")
					print("Debug Full Fish Name: "..tostring(fullFishName)..", Weight: "..tostring(weight))
					
					-- Fallback: Jika tidak ada kurung (ikan kecil/tanpa berat), anggap semua adalah nama
					if not fullFishName then
						fullFishName = fishPart
						weight = "N/A"
					end

					print("Debug After Fallback - Full Fish Name: "..tostring(fullFishName)..", Weight: "..tostring(weight))
					
					-- Bersihkan spasi berlebih di nama
					fullFishName = fullFishName:gsub("^%s*(.-)%s*$", "%1")
					
					-- 4. Deteksi Nama Ikan & Mutasi (MENGGUNAKAN SORTED CACHE)
					local fishId = nil
					local mutation = "-"
					
					-- Loop menggunakan list yang sudah di-sort (Panjang -> Pendek)
					-- Ini menjamin "Ancient Squid" terdeteksi sebelum "Squid"
					for _, item in ipairs(SortedFishCache) do
						-- Gunakan plain search
						if string.find(fullFishName, item.name, 1, true) then
							fishId = item.id
							
							-- Hapus nama ikan asli dari string penuh untuk mendapatkan mutasi
							-- Escape karakter magic agar gsub tidak error
							local escName = item.name:gsub("([%(%)%.%%%+%-%*%?%[%^%$])", "%%%1")
							
							-- Ganti nama ikan dengan kosong, sisanya adalah mutasi
							local cleanMutation = fullFishName:gsub(escName, ""):gsub("^%s*(.-)%s*$", "%1")
							
							if cleanMutation ~= "" then
								mutation = cleanMutation
							end
							
							-- Break agar tidak lanjut mencocokkan nama yang lebih pendek (misal: Squid)
							break 
						end
					end
					
					if fishId then
						return playerName, fishId, weight, mutation
					end
					return nil
				end

				-- [5] Helper: Clear Connections
				local function ClearConnections()
					for _, conn in pairs(WebhookState.connections) do
						if conn then conn:Disconnect() end
					end
					WebhookState.connections = {}
				end

				-- [6] Logic: Player Mode (Visual & Cutscene)
				local function ConnectPlayerMode()
					ClearConnections() -- Hapus koneksi sebelumnya
					...
					if event then
						local conn = event.OnClientEvent:Connect(function(fishId, weightData)
							-- Parsing Data (Sesuai snippet kamu)
							...
							-- Kirim Webhook
							-- Karena remote ini memberikan fishId langsung, kita tidak perlu mencari nama ikan dulu.
							SendWebhook(fishId, w, m, LocalPlayer)
						end)
						
						-- Masukkan ke table connections agar bisa dibersihkan saat toggle off/ganti mode
						table.insert(WebhookState.connections, conn)
					end
				end

				-- [7] Logic: Server Mode (SYSTEM MESSAGE ONLY)
				local function ConnectServerMode()
					...
					local SystemMsgEvent = net.net:FindFirstChild("RE/DisplaySystemMessage")
					
					if SystemMsgEvent then
						local conn = SystemMsgEvent.OnClientEvent:Connect(function(msg)
							-- 1. Parse Pesan Server (Hanya jika ada "obtained a")
							local pName, fId, w, mut = ParseSystemMessage(msg)
							
							if pName and fId then
								-- 2. Buat Fake Player Object
								local fakePlayer = {Name = pName, UserId = 0}
								local realPlayer = Players:FindFirstChild(pName)
								if realPlayer then fakePlayer = realPlayer end
								
								-- 3. Kirim Webhook (Filter Rarity ada di dalam fungsi SendWebhook)
								SendWebhook(fId, w, mut, fakePlayer)
							end
						end)
						table.insert(WebhookState.connections, conn)
					end
				...
				WebhookSection:AddInput("WebhookURL", {
					...
					Callback = function(v) WebhookState.url = v end
				})

				WebhookSection:AddDropdown("WebhookMode", {
					Title = "Detection Mode",
					Description = "Player = Only You | Server = Chat Global",
					...
					Callback = function(v)
						...
						if WebhookState.enabled then
							ConnectEvent() -- Restart connection with new mode
							Fluent:Notify({Title="Mode Changed", Content="Switched to "..v.." mode", Duration=3})
						end
					end
				})
				...
				WebhookSection:AddDropdown("WebhookRarity", {
					...
					Multi = true,
					Default = {},
					Callback = function(v) WebhookState.rarities = v or {} end
				})
				...
				WebhookSection:AddToggle("WebhookEnabled", {
					...
					Callback = function(v)
						...
						if v then
							ConnectEvent()
							Fluent:Notify({Title="Webhook Enabled", Content="Logger is active in "..WebhookState.mode.." mode!", Duration=3})
						else
							ClearConnections()
					...
					----------------------------------------------------
					-- 🌐 GLOBAL SECRET FISH WEBHOOK (Hidden System)
					----------------------------------------------------
					...
					----------------------------------------------------
					-- 💾 CONFIG SYSTEM (Fluent SaveManager)
					----------------------------------------------------
				...
				local function LoadUtilTab ()
					...
					local function HopLowServer()
						...
						print("📉 [Smart Low Hop] Mencari server sepi...")
						...
						while true do
							...
							if not success or not response or not response.Body then
								warn("⚠️ Koneksi timeout/error, mencoba lagi...")
								task.wait(3)
								continue
							end
							...
							if response.StatusCode == 429 then
								warn("⚠️ Terlalu cepat (Rate Limit)! Menunggu 5 detik...")
								task.wait(5)
								continue
							end
							...
							if not Site or not Site.data then
								warn("⚠️ Data kosong, mencoba ulang...")
								cursor = "" 
								task.wait(2)
								continue
							end
							...
							for _, v in pairs(Site.data) do
								...
								if v.playing and v.playing > 0 and v.id ~= game.JobId and v.playing < v.maxPlayers then
									...
									print("🚀 Mencoba join server sepi: " .. v.playing .. "/" .. v.maxPlayers .. " Players")
									...
									if tpSuccess then
										...
										task.wait(5)
										warn("⚠️ Teleport lambat, mencoba server sepi berikutnya...")
									end
								end
							end
							
							print("ℹ️ Halaman ini tidak ada server yang valid, lanjut ke halaman berikutnya...")
							
							if cursor == "" then
								print("🔄 Reset pencarian dari awal...")
							end
					...
					local function Rejoin()
						print("🔄 [Rejoin] Mencoba masuk ulang ke game...")
